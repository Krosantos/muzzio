...
const getMaindeckCount:GetCardCount = (cards) => cards.map((card) => ({ ...card, sideboardCount: 0 }));
const getSideboardCount:GetCardCount = (cards) => cards.map((card) => {
...
const appendCards:AppendCards = (sortedCards, format, commanderData, oathbreakerData) => {
