...
const calculateIdentity: CalculateIdentity = (oathbreakerData) => {
  ...
  if (identity.length < 1) identity.push("C");
  return identity;
};
...
const useOathbreaker: UseOathbreaker = () => {
  ...
  const setOathbreaker = useCallback(
    (oathbreaker) => {
  ...
  const setSignatureSpell = useCallback(
    (signatureSpell) => {
  ...
  const colorIdentity = useMemo(() => calculateIdentity(oathbreakerData), [
    oathbreakerData,
  ]);
  const oathbreaker = useMemo(() => get(oathbreakerData, "oathbreaker", {}), [
    oathbreakerData,
  ]);
  const signatureSpell = useMemo(() => get(oathbreakerData, "signatureSpell", {}), [
    oathbreakerData,
  ]);
