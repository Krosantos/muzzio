import React, {
  Reducer, useReducer, useMemo, useCallback,
} from 'react';
...
const SettingsProvider:React.FC = ({ children }) => {
  const [
    settings,
    dispatch,
  ] = useReducer<Reducer<Settings, Action>>(settingReducer, electronSettings.getSync() as Settings);
  const setSettings = useCallback((key, value) => {
    dispatch({ key, value });
  }, [dispatch]);
  const value = useMemo<SettingsContextValue>(() => ({ setSettings, settings }), [setSettings, settings]);
